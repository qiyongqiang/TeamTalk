        ...
        Collections.sort(contactList, new Comparator<UserEntity>() {
            ...
            public int compare(UserEntity entity1, UserEntity entity2) {
                ...
                if (entity1.getDepartmentId() == entity2.getDepartmentId()) {
                    //局领导 部门特殊处理 不按照拼音排序 按照指定顺序排列
                    if(departmentMap.get(entity1.getDepartmentId()).getDepartName().equals("局领导")){
                        List<String> leaderList = java.util.Arrays.asList(UrlConstant.LEADER_ARR);
                        int index1 = leaderList.indexOf(entity1.getMainName());
                        int index2 = leaderList.indexOf(entity2.getMainName());
...
//                        return dept1.getDepartName().compareToIgnoreCase(dept2.getDepartName());
                        //部门按照优先级排列
                        return dept1.getPriority()-dept2.getPriority();
                    } else
                        return 1;
